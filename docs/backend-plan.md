...
`requirements_v2.md`(Figma 전수 분석 + 기획 노트 기반 v2 요구사항 명세)를 바탕으로 백엔드 코드 계획을 구체화한다. 이번 개정(v2)에서는 다음 두 가지를 추가로 반영했다:

- **`컨벤션.md`** — 팀의 Java 코드 컨벤션(레이어별 메서드 명명, 디렉토리 구조, 에러 코드 포맷, DTO 필드명 규칙, 테스트 스타일)을 그대로 채택.
